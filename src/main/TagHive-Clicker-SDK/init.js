    ...
    try {
      ...
      if (!gotTheLock) {
        ...
        if (isElevatedResult) {
          ...
          var command = ['/usr/bin/pkexec', 'env', 'DISPLAY=$DISPLAY', 'XAUTHORITY=$XAUTHORITY'] // Command for running a process with elevated privileges
          var magic = 'SUDOPROMPT' // Magic string used for identifying the prompt
          command.push('/bin/bash -c "echo ' + magic.trim() + ' ; ' + execName + '"') // Adding the command to execute the main process with elevated privileges
      ...
      case 'write':
        if (classKeySerialPort && classKeySerialPort.isOpen) {
          ...
          classKeySerialPort.write(writeBuffer, function (err, result) {
            ...
            if (result) {
              console.log('Response received after sending message : ' + result) // Logging the response received after sending the message
            }
          })
        }
        break
